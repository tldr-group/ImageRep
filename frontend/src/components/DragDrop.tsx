...
const DragDrop = ({ loadFromFile }: DragDropProps): JSX.Element => {
  ...
  return (
    <div style={dragDropStyle} onDragOver={handleDrag} onDrop={handeDrop}>
      {!isMobile && (
        <div
          style={{
            display: "flex",
        ...
        >
          <span>
            Drag microstructure file, or view example{" "}
            <a
              style={{ cursor: "pointer", color: "blue" }}
              onClick={(e) => viewExample(DEFAULT_IMAGE_2D)}
            >
              in 2D
            </a>{" "}
            or{" "}
            <a
              style={{ cursor: "pointer", color: "blue" }}
              onClick={(e) => viewExample(DEFAULT_IMAGE_3D)}
            >
              {" "}
              3D
            </a>
          ...
          <span style={{ fontSize: "0.7em" }}>
            (image must be segmented & image background (e.g. scale bar) cropped
            out)
          </span>
        </div>
      )}
