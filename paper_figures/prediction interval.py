...
def plot_likelihood_of_phi(image_pf, pred_std, std_dist_std):
    ...
    ax[0,2].plot(pf_x_1d, mid_std_dist, c='orange', label='Likelihood of true phase \nfraction, completely trusting\nstd prediction as true std')
    ...
    ax[1,0].hlines(pred_std, pf_x_bounds[0], pf_x_bounds[1], colors='orange', label = "(c) - Likelihood of true phase\nfraction using predicted std")
    ax[1,0].set_title('(d)')
    ...
    contour = ax[1,1].contourf(pf_mesh, std_mesh, pf_dist, levels=100, cmap = 'plasma') 
    ax[1,1].set_title('(e)')
    ...
    ax[1,2].plot(pf_x_1d, sum_dist_norm, c='blue', label = "Likelihood of true phase fraction\nusing integration of (e)'s rows")
