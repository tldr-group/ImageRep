...
for j, p in enumerate(projects):
    ...
    smaller_reps = 40  # TODO change this
    img = util.generate_image(netG, p, lf=lf, threed=mode=='3D', reps=smaller_reps)
    print(img.size())
    if img.any():
        ...
        err_exp_vf, exp_ir_vf = util.stat_analysis_error(img[0], edge_lengths, img_dims, compared_shape, vf, threed=mode=='3D', conf=0.95)
        ...
        err_exp_sa, exp_ir_sa = util.stat_analysis_error(util.sa_map_from_sas(sa_images), edge_lengths, img_dims, compared_shape, sa, threed=mode=='3D', conf=0.95)
        ...
        print()
        with open(f"datafin_new4.json", "w") as fp:
