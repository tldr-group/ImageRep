...
for pths, c in zip(['Graphite/Calendering','NMC/Calendering'], ['g', 'b']):
    ...
    for i, d in enumerate(d):  
        ...
        for ph in np.unique(img):
            ...
            imph[img==ph]=1
            vf = np.mean(imph)
            err, n, tpc = make_error_prediction(imph[0], correction=True, model_error=False, mxtpc=50)
...
ax.scatter(errs_stat[40:],errs_tpc[40:], label='NMC')
...
ax.set_aspect(1)
ax.plot(np.arange(20), np.arange(20), c='black')
ax.legend()
