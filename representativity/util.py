...
def tpc_radial(img, mx=100, threed=False, periodic=True):
    ...
    return two_point_correlation(
        img, desired_length=desired_length, periodic=periodic, threed=threed
    )
...
def tpc_to_cls(tpc, im, im_shape):
    ...
    coeff = calc_coeff_for_cls_prediction(
        norm_vol, dist_arr, end_dist, img_volume, bool_array
    )
    ...
    while pred_is_off:
        ...
        tpc, pred_cls = change_pred_cls(
            coeff, tpc, pf, pf_squared, bool_array, im_shape, sign
        )
        pred_is_off, sign = pred_cls_is_off(pred_cls, im, pf)
    return pred_cls
...
def make_error_prediction(
    ...
    if model_error:
        ...
        abs_err_for_img = minimize(
            optimize_error_conf_pred, conf**0.5, args, bounds=bounds
        ).fun
        args = (conf, std_model, pf, abs_err_target)
        n_for_err_targ = minimize(
            optimize_error_n_pred, conf**0.5, args, bounds=bounds
        ).fun
...
def calc_autocorrelation_orthant(img, numel_large, dims, desired_length=100):
    ...
    tpc = (
        np.fft.irfftn(img_FFT.conjugate() * img_FFT, s=img.shape, axes=ax).real
        / numel_large
    )
