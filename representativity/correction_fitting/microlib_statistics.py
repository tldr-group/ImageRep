...
CWD = os.getcwd()
DATA_PATH = os.path.join(CWD, "representativity/correction_fitting")
...
def run_ir_prediction(netG, n, mode, imsize, all_data, conf, run_number=0):
    ...
    if single_img.any():
        single_img = single_img.cpu().numpy()[0]

        out = core.make_error_prediction(single_img, confidence=conf, model_error=False)
        pred_err_vf, pred_ir_vf = out["percent_err"], out["integral_range"]

        print(f"pred ir {imsize} = {np.round(pred_ir_vf, 3)}")
        im_vf = single_img.mean().cpu().item()
        one_im_stat_pred = core.stat_analysis_error_classic(single_img, im_vf)
...
def run_statistical_fit_analysis(netG, n, mode, edge_lengths_fit, all_data):
    ...
    fit_ir_vf_classic = core.stat_analysis_error_classic(many_imgs.cpu().numpy(), vf)
...
def compare_statistical_fit_error(
    ...
    img_sizes = [np.zeros((n_dims)) + l for l in edge_lengths_pred]
    fit_errs_vf = core.bernouli(
        vf, core.n_samples_from_dims(img_sizes, fit_ir_vf), conf=conf
    )
...
def json_preprocessing(microlib_path: str = "/home/amir/microlibDataset/"):
    ...
    # TODO: fix this - paths will be wrong
    micros = [f"{microlib_path}{p}/{p}" for p in micro_names]
...
def run_microlib_statistics(
    ...
    run_p=True,
    run_number=0,
    microlib_path: str = "/home/amir/microlibDataset/",
):
    """
    ...
    all_data, micros, netG, v_names, run_v_names = json_preprocessing(microlib_path)
    ...
    for _, p in enumerate(micros):
        ...
        for mode in cur_modes:
            ...
            with open(f"{DATA_PATH}/microlib_statistics_periodic.json", "w") as fp:
...
def main_run_microlib_statistics(
    ...
    if run_s:
        ...
        run_microlib_statistics(
            cur_modes=cur_modes, run_s=True, run_p=False, microlib_path=microlib_path
        )
