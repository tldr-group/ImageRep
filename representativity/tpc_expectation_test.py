...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplot_mosaic([['circle0', 'circle1', 'TPC figure', 'TPC figure'],
    ...
    plt.figtext(0.31, 0.9, f'4 random {imsize}$^2$ images with $E[\Phi]$ = {pf} and circle diameter = {circle_size*2}', ha='center', va='center', fontsize=12)
    ...
    len_tpc = len(tpc_results[0])
    fill_1 = tpc_fig.fill_between(np.arange(len_tpc), mean_tpc,[pf_squared]*len_tpc, alpha=0.2, 
                              where=mean_tpc>=pf_squared,label = f'Area $A_1$')
    fill_2 = tpc_fig.fill_between(np.arange(len_tpc), [pf_squared]*len_tpc,mean_tpc, alpha=0.2, 
                              where=np.logical_and(dist_len<=circle_size*2, np.array(mean_tpc<pf_squared)),label = f'Area $A_2$')
    fill_3 = tpc_fig.fill_between(np.arange(len_tpc),[pf_squared]*len_tpc, mean_tpc, alpha=0.2, 
    ...
    tpc_fig.text(22, pf_squared+text_jump*7, r'How the model predicts the variance of $\Phi$:', fontsize=12)
    tpc_fig.text(22, pf_squared+text_jump*6, '$Var[\Phi]=E[\Phi^2]-E[\Phi]^2$', fontsize=12)
    tpc_fig.text(22, pf_squared+text_jump*5, r"$Var[\Phi]=\frac{1}{C_{40}}\cdot B$ (Normalization of B's width)", fontsize=12)
    tpc_fig.text(22, pf_squared+text_jump*4, r'$\sum_r{(E[T_r]-E[\Phi^2])}=0$, So', fontsize=12)
    ...
    tpc_fig.text(22, pf_squared+text_jump*1, r'$Var[\Phi]=\frac{1}{C_{40}}\cdot (A_1-A_2)=E[\Psi]$', fontsize=12)
    ...
    tpc_fig.set_yticks([pf**2, np.round(pf_squared, 4), pf], [pf**2, np.round(pf_squared, 4), pf])
    tpc_fig.legend()
