...
def error_by_size_estimation(dim, run_number=0, std_not_cls=True):
    ...
    for edge_length in edge_lengths_pred:
        _, _, _, err, std, pfs = comparison_results(data_dim, micro_names, dim,
...
def comparison_results(data_dim, micro_names, dim, edge_length, run_number, std_not_cls=False):
    ...
    pred_cls_all = np.array([m_data[f'run_{run_number}']['pred_ir_vf'][edge_length] for m_data in micros_data])
    pred_cls_oi_all = np.array([m_data[f'run_{run_number}']['pred_ir_one_im_fit_vf'][edge_length] for m_data in micros_data])
    ...
    if std_not_cls:
        ...
        pred_cls_all = ((pred_cls_all/edge_length)**dim_int*pfs_one_minus_pfs)**0.5
        fit_cls_all = ((fit_cls_all/edge_length)**dim_int*pfs_one_minus_pfs)**0.5
    # ir_results = [fit_data_all, pred_data_all, pred_ir_oi_pf]

    errs = (pred_cls_all-fit_cls_all)/pred_cls_all  # percentage error of the prediction
...
def pred_vs_fit_all_data(dim, edge_length, num_runs=5, std_not_cls=True):
    ...
    for i in range(num_runs):
        ...
        pred_data, fit_data, pred_data_oi, _, std, pfs = comparison_results(*data_micro, dim, edge_length, i, std_not_cls=std_not_cls)
...
def plot_pred_vs_fit(dim, edge_length, num_runs=5, std_not_cls=True):
    pred_data_all, fit_data_all, _, _, pfs = pred_vs_fit_all_data(dim, edge_length, num_runs, std_not_cls=std_not_cls)
...
def optimal_slopes(dim, num_runs=5):
    ...
    stds = []
    for edge_length in edge_lengths_pred:
        pred_data, fit_data, _, std, pfs = pred_vs_fit_all_data(dim, str(edge_length), num_runs)
        stds.append(std)
