...
def compare_statistical_fit_error(n, mode, edge_lengths_pred, fit_ir_vf, vf, all_data, conf):
    '''
...
def run_microlib_statistics(cur_modes=['2D', '3D'], run_s=False, run_p=True, run_number=0):
    '''
    ...
    for _, p in enumerate(micros):
        ...
        for mode in cur_modes:
            ...
            n = p.split('/')[-1]
            insert_v_names_in_all_data(all_data, mode, n, v_names, run_v_names, run_number)  # insert var names in all_data
