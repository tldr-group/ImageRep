...
def run_ir_prediction(netG, p, n, mode, imsize, conf, datafin):
    '''
    This function is used to predict the integral range of the microstructure.'''
    lf = imsize//32 + 2  # the size of G's input
    single_img = util.generate_image(netG, p, lf=lf, threed=mode=='3D', reps=1)
    pred_err_vf, _, pred_ir_vf = util.make_error_prediction(single_img, conf=conf, model_error=False, correction=False, mxtpc=100)
    print(f'pred ir = {pred_ir_vf}')
    datafin[f'data_gen_{mode}'][n]['pred_ir_vf'][imsize] = pred_ir_vf
    datafin[f'data_gen_{mode}'][n]['pred_err_vf'][imsize] = pred_err_vf*100

def run_statistical_fit_analysis(netG, p, n, mode, edge_lengths_fit, datafin):
    
    imsize = 448 if mode=='3D' else 1600
    lf = imsize//32 + 2  # the size of G's input
    many_imgs = util.generate_image(netG, p, lf=lf, threed=mode=='3D', reps=50)
    vf = torch.mean(many_imgs).cpu().item()
    print(f'{n} vf = {vf}')
    datafin[f'data_gen_{mode}'][n]['vf'] = vf
    fit_ir_vf = util.stat_analysis_error(many_imgs, edge_lengths_fit)
...
def compare_to_prediction(n, mode, edge_lengths_pred, fit_ir_vf, vf, datafin, conf):
    ...
    for i in range(len(edge_lengths_pred)):
        ...
        datafin[f'data_gen_{mode}'][n]['fit_err_vf'][imsize] = fit_errs_vf[i]*100
        print(f'{n} fit error vf = {fit_errs_vf[i]*100}')
...
def run_microlib_statistics():
    ...
    datafin['data_gen_2D']['edge_lengths_fit'] = torch.arange(500, 1000, 20)
    datafin['data_gen_3D']['edge_lengths_fit'] = torch.arange(120, 400, 20)
        ...
        for mode in cur_modes:
            ...
            if run_prediction:
                for imsize in edge_lengths_pred:
                    run_ir_prediction(netG, p, n, mode, imsize, datafin, conf)

            if run_statistical_fit:
                fit_ir_vf, vf = run_statistical_fit_analysis(p, n, mode, edge_lengths_fit, datafin)
            ...
            print(f'time for one micro {mode} = {np.round((time.time()-t_before)/60, 2)} minutes')
...
# fit_err_vfs.append(fit_err_vf)
                ...
                # print(f'% diff vf = {(err_fit_vf-pred_err_vf*100)/err_fit_vf}')
                # print()
                ...
                # print(f'% diff = {(err_fit_sa-pred_err_sa*100)/err_fit_sa}')
