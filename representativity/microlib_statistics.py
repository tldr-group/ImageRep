...
v_names = ['vf', 'pred_ir_vf', 'pred_err_vf', 'exp_ir_vf', 'exp_err_vf']
...
for mode in modes:
    ...
    for v_name in v_names:
        if v_name not in datafin[f'data_gen_{mode}']:
            datafin[f'data_gen_{mode}'][v_name] = {}

cur_modes = modes  # change to ['2D'] or ['3D'] to only do one mode

# Edge lengths for the experimental statistical analysis:
datafin['data_gen_2D']['edge_lengths_exp'] = torch.arange(500, 1000, 20)
datafin['data_gen_3D']['edge_lengths_exp'] = torch.arange(120, 400, 20)

# Edge lengths for the predicted integral range:
datafin['data_gen_2D']['edge_lengths_pred'] = torch.arange(8, 65, 4)*32
datafin['data_gen_3D']['edge_lengths_pred'] = torch.arange(4, 19, 1)*32

for j, p in enumerate(micros):
    for mode in cur_modes:
        edge_lengths_exp = datafin[f'data_gen_{mode}']['edge_lengths_exp']
        ...
        img = util.generate_image(netG, p, lf=lf, threed=mode=='3D', reps=1)
        print(mode)
        print(img.size())
        if img.any():
            ...
            datafin[f'data_gen_{mode}'][n]['pred_ir_vf'] = np.mean(pred_ir_vfs)
            datafin[f'data_gen_{mode}'][n]['pred_ir_vf'] = np.mean(pred_ir_vfs)
            if mode=='3D':
                datafin[f'data_gen_{mode}'][n]['dim_variation'] = 0
            else:
                datafin[f'data_gen_{mode}'][n]['dim_variation'] = np.var(pred_ir_vfs)/np.mean(pred_ir_vfs)
                print(f'dim variation = {np.var(pred_ir_vfs)/np.mean(pred_ir_vfs)}')
            datafin[f'data_gen_{mode}'][n]['pred_err_vf'] = np.mean(pred_err_vfs).astype(np.float64)*100
...
# print(errs_ir_vf)
        ...
        #     print(f'mean ir = {exp_ir_vf}')
            ...
            # datafin[f'data_gen_{mode}'][n]['pred_err_sa'] = pred_err_sa.astype(np.float64)*100
            # datafin[f'data_gen_{mode}'][n]['pred_ir_sa'] = pred_ir_sa
            # datafin[f'data_gen_{mode}'][n]['err_exp_sa'] = err_exp_sa.item()
            # datafin[f'data_gen_{mode}'][n]['exp_ir_sa'] = exp_ir_sa
...
# print(pred_ir_sas)
            ...
            # datafin[f'data_gen_{mode}'][n]['pred_err_sa'] = np.mean(pred_err_sas).astype(np.float64)*100

            # print(exp_ir_vfs)
            # print(f'mean ir = {np.mean(exp_ir_vfs)}')
            # datafin[f'data_gen_{mode}'][n]['exp_ir_vf'] = np.mean(exp_ir_vfs)
            # datafin[f'data_gen_{mode}'][n]['exp_ir_vf'] = np.mean(exp_ir_vfs)
            # if mode=='3D':
            #     datafin[f'data_gen_{mode}'][n]['exp_dim_variation'] = 0
            # else:
            #     datafin[f'data_gen_{mode}'][n]['exp_dim_variation'] = np.var(exp_ir_vfs)/np.mean(exp_ir_vfs)
            #     print(f'dim variation = {np.var(exp_ir_vfs)/np.mean(exp_ir_vfs)}')
            # datafin[f'data_gen_{mode}'][n]['exp_err_vf'] = np.mean(exp_err_vfs).astype(np.float64)

            # print(exp_ir_sas)
            # print(f'mean ir = {np.mean(exp_ir_sas)}')
            # datafin[f'data_gen_{mode}'][n]['exp_ir_sa'] = np.mean(exp_ir_sas)
            # if mode=='3D':
            #     datafin[f'data_gen_{mode}'][n]['exp_dim_variation'] = 0
            # else:
            #     datafin[f'data_gen_{mode}'][n]['exp_dim_variation'] = np.var(exp_ir_sas)/np.mean(exp_ir_sas)
            #     print(f'dim variation = {np.var(exp_ir_sas)/np.mean(exp_ir_sas)}')
            # datafin[f'data_gen_{mode}'][n]['exp_err_sa'] = np.mean(exp_err_sas).astype(np.float64)

            # datafin[f'data_gen_{mode}'][n]['err_exp_vf'] = err_exp_vf.item()
            # datafin[f'data_gen_{mode}'][n]['exp_ir_vf'] = exp_ir_vf
