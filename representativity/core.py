...
def classical_approach_test(binary_image: np.ndarray, n_dims: int) -> bool:
    ...
    if n_dims == 3:
        if im_size > SMALL_IM_3D:
            ...
            return False
    if VERBOSE:
        print(f"Image is too small with ({im_size} elements, {n_dims}D), comparing to classical approach")
    return True
